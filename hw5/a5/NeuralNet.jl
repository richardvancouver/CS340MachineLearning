...
function NeuralNet_backprop(bigW,x,y,nHidden)
	...
	for layer in 2:nLayers 
		...
		# @show(reshape(bigW[ind+1:ind+nHidden[layer]*nHidden[layer-1]],nHidden[layer],nHidden[layer-1]))
		#@show(bigW)
	end
	...
	h(z) = 1./(1+exp.(-z))#tanh.(z)#1./(1+exp.(-z))#log.(1+exp.(z))#1./(1+exp.(-z))#tanh.(z)#1./(1+exp.(-z)) #tanh.(z)#
	dh(z) = exp.(-z)./(1+exp.(-z)).^2#(sech.(z)).^2#exp.(-z)./(1+exp.(-z)).^2#1./(1+exp.(-z))#exp.(-z)./(1+exp.(-z)).^2#(sech.(z)).^2#exp.(-z)./(1+exp.(-z)).^2 #(sech.(z)).^2#
...
function NeuralNet_predict(bigW,Xhat,nHidden)
	...
	#### Define activation function and its derivative
	h(z) = 1./(1+exp.(-z))#tanh.(z)#
	dh(z) = exp.(-z)./(1+exp.(-z)).^2#(sech.(z)).^2#
